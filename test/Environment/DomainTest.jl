...
@testset "Domain" begin
    ...
    @test EtherParticlesGPU.Environment.get_n_x(domain_2d) == 26
    @test EtherParticlesGPU.Environment.get_n_y(domain_2d) == 33
    @test EtherParticlesGPU.Environment.get_n(domain_2d) == 26 * 33
    ...
    @test EtherParticlesGPU.Environment.get_gap_x(domain_2d) ≈ (x_1 - x_0) / 26
    @test EtherParticlesGPU.Environment.get_gap_y(domain_2d) ≈ (y_1 - y_0) / 33
    ...
    @test EtherParticlesGPU.indexCartesianToLinear(domain_2d, IT(2), IT(3)) == 2 + (3 - 1) * 26
    @test EtherParticlesGPU.indexLinearToCartesian(domain_2d, IT(2 + (3 - 1) * 26)) == (2, 3)
    ...
    @test EtherParticlesGPU.indexLinearFromPosition(domain_2d, 1.5f0, 0.3f0) == 17 + (16 - 1) * 26
end
